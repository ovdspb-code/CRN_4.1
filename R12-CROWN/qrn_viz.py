...
def plot_energy_sweep(energy_df, save_path: str) -> None:
    ...
    ax2 = ax1.twinx()
    ax2.loglog(t_reset, tax_ratio, linestyle="--", linewidth=2, label=r"$E_{tax}/E_{comm}$")
    ax2.set_ylabel(r"Tax ratio $E_{tax}/E_{comm}$")
    ...
    ax2.text(float(t_reset.min()), 1.15e-1, "Red line (0.1)", fontsize=8)
    ax2.text(float(t_reset.min()), 4e-3, "Safe zone (<0.01)", fontsize=8)
